...
export default function PropertyModal({ property, isOpen, onClose }: PropertyModalProps) {
  ...
  const openMapInNewTab = () => {
    ...
    const params = new URLSearchParams({
      ...
      priceLabel: buildPriceLabel(property),
    });
  ...
  return (
    <div
      className={`fixed inset-0 z-[90] flex items-center justify-center p-4 transition-[background-color,opacity] duration-300 ${
        isOpen
          ? "property-modal-backdrop-enter bg-slate-950/24 opacity-100 backdrop-blur-[3px]"
          : "pointer-events-none bg-slate-950/0 opacity-0 backdrop-blur-0"
      }`}
      aria-hidden={!isOpen}
    >
      <button type="button" onClick={onClose} className="absolute inset-0" aria-label="ปิดรายละเอียดบ้าน" />

      <div
        className={`relative z-[1] w-full max-w-4xl overflow-hidden rounded-[1.6rem] bg-white shadow-[0_35px_90px_-36px_rgba(15,23,42,0.7)] transition-[transform,opacity] duration-300 ${
          isOpen ? "property-modal-enter translate-y-0 scale-100 opacity-100" : "translate-y-6 scale-[0.985] opacity-0"
        }`}
      >
        <div className="relative max-h-[90vh] overflow-y-auto">
          <div className="sticky top-0 z-30 border-b border-gray-200 bg-white/95 px-6 py-4 backdrop-blur-sm flex justify-between items-center">
